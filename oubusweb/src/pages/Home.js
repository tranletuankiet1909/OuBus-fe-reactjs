...
const Home = () => {
  ...
  useEffect(() => {
    ...
    console.info('maxpage', maxPage, page)
  ...
  return token ? (
    <Box style={{ margin: 10 }}>
      ...
      <TablePagination
        component="div"
        count={totalCount}
        page={page} // Hiển thị đúng trang, chuyển từ API (1-based) sang giao diện (0-based)
...
//       let url = `https://tltk.pythonanywhere.com/bustrips/?q=${q}&route_id=${routeId}&page=${page}&start_time=${formattedDate}`;
//       console.info(url);
//       try {
//           setLoading(true);
